...
class trainavoidbed(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = gym.spaces.MultiDiscrete([21, 11])
    ...
    def step(self,action):
        ...
        time.sleep(1)  # Sleep for 1.5 seconds to allow for color change
        print("after sleep")
        if bedrock > 50 and mithril + titanium == 0:
            print("Bedrock detected, moving back")
            reward += 2
        elif mithril > 50 and bedrock + titanium == 0:
            print("Mithril detected, moving back")
            reward -= 2
        self.current_step += 1
...
def main():
    ...
    if os.path.exists("mithrilminingPPO.zip"):
        ...
        model = PPO("CnnPolicy", env, verbose=2, ent_coef=0.01, n_steps=32, batch_size=32, learning_rate=0.005, tensorboard_log="./logs/")
    callback = StopTrainingCallback()
    print("training started!")
    model.learn(total_timesteps= 2560, tb_log_name="run_33",callback=callback, log_interval=1) 
